...
export default function OpportunityCard({ item }: { item: Opportunity }) {
  const deadlineLabel = item.deadline ? new Date(item.deadline).toLocaleDateString() : 'Open'
  return (
    <article className="group rounded-2xl border border-slate-100 bg-white p-4 shadow-sm">
      <div className="flex items-start gap-4">
        ...
        <div className="flex-1">
          ...
          <div className="mt-2 flex flex-wrap items-center gap-2">
            ...
            {item.status && item.status !== 'published' && <span className="rounded-full bg-amber-100 px-2 py-1 text-xs font-semibold text-amber-700">{item.status}</span>}
      ...
      <div className="mt-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div className="text-sm text-slate-600">Deadline: <span className="text-slate-900 font-semibold">{deadlineLabel}</span></div>
